...
export class ForecastEngine {
  static calculateForecast(context: FinancialContext): ForecastResult {
    ...
    const dailyAvgExpense = context.totalExpenses / 30
    const projectedExpenses = Math.round(context.totalExpenses + dailyAvgExpense * 5)
    ...
    const budgetRiskScore = context.budgetSummary.overspendingCategories.length > 0 ? 75 : 15
